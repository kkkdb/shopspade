...
const Axios = axios.create({
  ...
  baseURL: process.env.NODE_ENV === 'development' ? location.origin : '/',
})
// 超时时间（ms）
...
Axios.interceptors.request.use(
  (config) => {
    const token = getCookie('d_session_id') || getCookie('Authorization')
    if (token) {
    ...
    config.data.merchant_code = localStorage.getItem('merchantCode')
    return config
  },
  (error) => {
    return error
...
function goToLogin() {
  ...
  Modal.confirm({
    ...
    content: t('common.sessionExpiredMsg'),
